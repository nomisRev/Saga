 ...
 * Or when you need to compose multiple `actions` with a `compensation` that needs to run in a transaction like style.
 *
...
public sealed interface Saga<A> {
   ...
   * A single [Saga] can have many compensating actions, they will be composed in a FILO order.
   * This makes sure they're executed in reverse order as the actions.
   *
   * ```kotlin
   * saga {
  ...
  /**
   * Runs multiple [Saga]s in parallel and combines the result with the [transform] function.
   * When one of the two [Saga] fails then it will cancel the other,
   *  if the other [Saga] already finished then its compensating action will be run.
   *
  ...
  public class Full<A>(public val action: suspend SagaEffect.() -> A, public val compensation: suspend (A) -> Unit) : Saga<A>
 ...
 * The `saga` builder uses the suspension system to run actions, and automatically register their compensating actions.
 *
 * When the resulting [Saga] fails it will run all the required compensating actions,
 * also when the [Saga] gets cancelled it will respect its compensating actions before returning.
 *
 ...
 * Handy when you need to process `List<Saga<A>>` that might be coming from another layer.
 *
